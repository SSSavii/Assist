(globalThis.TURBOPACK = globalThis.TURBOPACK || []).push([typeof document === "object" ? document.currentScript : undefined, {

"[project]/src/app/(main)/auction/page.tsx [app-client] (ecmascript)": ((__turbopack_context__) => {
"use strict";

var { g: global, __dirname, k: __turbopack_refresh__, m: module } = __turbopack_context__;
{
__turbopack_context__.s({
    "default": (()=>ShopPage)
});
var __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$compiled$2f$react$2f$jsx$2d$dev$2d$runtime$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__ = __turbopack_context__.i("[project]/node_modules/next/dist/compiled/react/jsx-dev-runtime.js [app-client] (ecmascript)");
var __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$compiled$2f$react$2f$index$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__ = __turbopack_context__.i("[project]/node_modules/next/dist/compiled/react/index.js [app-client] (ecmascript)");
;
var _s = __turbopack_context__.k.signature();
'use client';
;
// Все возможные призы
const ALL_PRIZES = [
    // Редкие призы (малый шанс)
    {
        name: 'Онлайн-разбор с Иваном',
        type: 'rare'
    },
    {
        name: 'Приоритетное место у Ивана',
        type: 'rare'
    },
    {
        name: 'Розыгрыш завтрака с Иваном',
        type: 'rare'
    },
    {
        name: 'Ответ Ивана голосом',
        type: 'rare'
    },
    {
        name: 'Звонок 1 на 1 с Антоном',
        type: 'rare'
    },
    // Обычные призы (хороший шанс)
    {
        name: '3 чек-листа',
        type: 'common'
    },
    {
        name: 'Созвон с Антоном и БА',
        type: 'common'
    },
    {
        name: '1000 A+',
        type: 'common'
    },
    {
        name: 'Разбор резюме',
        type: 'common'
    },
    {
        name: '500 A+',
        type: 'common'
    },
    {
        name: 'Консультация 15 мин',
        type: 'common'
    },
    {
        name: 'Доступ к материалам',
        type: 'common'
    },
    {
        name: 'Персональная рекомендация',
        type: 'common'
    }
];
function ShopPage() {
    _s();
    const [userPluses, setUserPluses] = (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$compiled$2f$react$2f$index$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["useState"])(2);
    const [isSpinning, setIsSpinning] = (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$compiled$2f$react$2f$index$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["useState"])(false);
    // eslint-disable-next-line @typescript-eslint/no-unused-vars
    const [winningPrize, setWinningPrize] = (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$compiled$2f$react$2f$index$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["useState"])(null);
    const [spinKey, setSpinKey] = (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$compiled$2f$react$2f$index$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["useState"])(0);
    const [slots, setSlots] = (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$compiled$2f$react$2f$index$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["useState"])([
        '',
        '',
        ''
    ]);
    // Функция для получения случайного приза с учетом вероятности
    const getRandomPrize = (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$compiled$2f$react$2f$index$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["useCallback"])({
        "ShopPage.useCallback[getRandomPrize]": ()=>{
            const random = Math.random();
            // 20% шанс на редкий приз, 80% на обычный
            const availablePrizes = random < 0.2 ? ALL_PRIZES.filter({
                "ShopPage.useCallback[getRandomPrize]": (p)=>p.type === 'rare'
            }["ShopPage.useCallback[getRandomPrize]"]) : ALL_PRIZES.filter({
                "ShopPage.useCallback[getRandomPrize]": (p)=>p.type === 'common'
            }["ShopPage.useCallback[getRandomPrize]"]);
            return availablePrizes[Math.floor(Math.random() * availablePrizes.length)];
        }
    }["ShopPage.useCallback[getRandomPrize]"], []);
    // Анимация прокрутки слотов
    const animateSlots = (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$compiled$2f$react$2f$index$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["useCallback"])({
        "ShopPage.useCallback[animateSlots]": (finalPrize)=>{
            const prizeIndex = ALL_PRIZES.findIndex({
                "ShopPage.useCallback[animateSlots].prizeIndex": (p)=>p.name === finalPrize.name
            }["ShopPage.useCallback[animateSlots].prizeIndex"]);
            const prizePositions = [
                (prizeIndex - 1 + ALL_PRIZES.length) % ALL_PRIZES.length,
                prizeIndex,
                (prizeIndex + 1) % ALL_PRIZES.length
            ];
            // Анимация промежуточных состояний
            let iteration = 0;
            const maxIterations = 20;
            const interval = setInterval({
                "ShopPage.useCallback[animateSlots].interval": ()=>{
                    iteration++;
                    if (iteration < maxIterations) {
                        // Быстрая прокрутка случайных значений
                        setSlots([
                            ALL_PRIZES[Math.floor(Math.random() * ALL_PRIZES.length)].name,
                            ALL_PRIZES[Math.floor(Math.random() * ALL_PRIZES.length)].name,
                            ALL_PRIZES[Math.floor(Math.random() * ALL_PRIZES.length)].name
                        ]);
                    } else {
                        // Финальные значения
                        setSlots(prizePositions.map({
                            "ShopPage.useCallback[animateSlots].interval": (pos)=>ALL_PRIZES[pos].name
                        }["ShopPage.useCallback[animateSlots].interval"]));
                        clearInterval(interval);
                        setIsSpinning(false);
                        // Показать выигрыш
                        setTimeout({
                            "ShopPage.useCallback[animateSlots].interval": ()=>{
                                setWinningPrize(finalPrize);
                                alert(`Поздравляем! Вы выиграли: ${finalPrize.name}`);
                            }
                        }["ShopPage.useCallback[animateSlots].interval"], 500);
                    }
                }
            }["ShopPage.useCallback[animateSlots].interval"], 100);
        }
    }["ShopPage.useCallback[animateSlots]"], []);
    const handleSpin = ()=>{
        if (isSpinning || userPluses < 1) return;
        setIsSpinning(true);
        setWinningPrize(null);
        setSpinKey((prev)=>prev + 1);
        // Определяем выигрыш
        const prize = getRandomPrize();
        // Запускаем анимацию
        animateSlots(prize);
        // Обновляем баланс
        setUserPluses((prev)=>prev - 1);
    };
    // Инициализация начальных значений слотов
    (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$compiled$2f$react$2f$index$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["useEffect"])({
        "ShopPage.useEffect": ()=>{
            setSlots([
                ALL_PRIZES[0].name,
                ALL_PRIZES[1].name,
                ALL_PRIZES[2].name
            ]);
        }
    }["ShopPage.useEffect"], []);
    return /*#__PURE__*/ (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$compiled$2f$react$2f$jsx$2d$dev$2d$runtime$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["jsxDEV"])("div", {
        className: "flex flex-col min-h-screen font-sans items-center px-4 pt-6 pb-4 text-center text-black bg-gray-50",
        children: [
            /*#__PURE__*/ (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$compiled$2f$react$2f$jsx$2d$dev$2d$runtime$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["jsxDEV"])("div", {
                className: "w-full mb-6",
                children: [
                    /*#__PURE__*/ (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$compiled$2f$react$2f$jsx$2d$dev$2d$runtime$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["jsxDEV"])("h1", {
                        className: "text-2xl font-bold mb-2",
                        children: "Магазин"
                    }, void 0, false, {
                        fileName: "[project]/src/app/(main)/auction/page.tsx",
                        lineNumber: 117,
                        columnNumber: 9
                    }, this),
                    /*#__PURE__*/ (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$compiled$2f$react$2f$jsx$2d$dev$2d$runtime$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["jsxDEV"])("p", {
                        className: "text-gray-600",
                        children: "Обменивай свои плюсы на интересные товары!"
                    }, void 0, false, {
                        fileName: "[project]/src/app/(main)/auction/page.tsx",
                        lineNumber: 120,
                        columnNumber: 9
                    }, this)
                ]
            }, void 0, true, {
                fileName: "[project]/src/app/(main)/auction/page.tsx",
                lineNumber: 116,
                columnNumber: 7
            }, this),
            /*#__PURE__*/ (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$compiled$2f$react$2f$jsx$2d$dev$2d$runtime$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["jsxDEV"])("div", {
                className: "w-full bg-white rounded-xl shadow-sm p-5 mb-5",
                children: /*#__PURE__*/ (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$compiled$2f$react$2f$jsx$2d$dev$2d$runtime$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["jsxDEV"])("div", {
                    className: "text-gray-700 mb-4",
                    children: [
                        "У вас доступно ",
                        /*#__PURE__*/ (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$compiled$2f$react$2f$jsx$2d$dev$2d$runtime$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["jsxDEV"])("span", {
                            className: "font-semibold",
                            children: [
                                userPluses,
                                " плюсов"
                            ]
                        }, void 0, true, {
                            fileName: "[project]/src/app/(main)/auction/page.tsx",
                            lineNumber: 128,
                            columnNumber: 26
                        }, this)
                    ]
                }, void 0, true, {
                    fileName: "[project]/src/app/(main)/auction/page.tsx",
                    lineNumber: 127,
                    columnNumber: 9
                }, this)
            }, void 0, false, {
                fileName: "[project]/src/app/(main)/auction/page.tsx",
                lineNumber: 126,
                columnNumber: 7
            }, this),
            /*#__PURE__*/ (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$compiled$2f$react$2f$jsx$2d$dev$2d$runtime$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["jsxDEV"])("div", {
                className: "w-full bg-white rounded-xl shadow-sm p-4 mb-5",
                children: [
                    /*#__PURE__*/ (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$compiled$2f$react$2f$jsx$2d$dev$2d$runtime$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["jsxDEV"])("div", {
                        className: "flex justify-center items-center h-32 mb-4 bg-gradient-to-br from-purple-100 to-blue-100 rounded-lg overflow-hidden border-2 border-purple-200",
                        children: /*#__PURE__*/ (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$compiled$2f$react$2f$jsx$2d$dev$2d$runtime$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["jsxDEV"])("div", {
                            className: "flex space-x-3",
                            children: slots.map((slot, index)=>/*#__PURE__*/ (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$compiled$2f$react$2f$jsx$2d$dev$2d$runtime$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["jsxDEV"])("div", {
                                    className: "w-28 h-28 flex items-center justify-center bg-white border-2 border-purple-300 rounded-xl shadow-inner text-xs font-medium p-2 text-center transition-all duration-200 transform hover:scale-105",
                                    children: slot
                                }, `${spinKey}-${index}`, false, {
                                    fileName: "[project]/src/app/(main)/auction/page.tsx",
                                    lineNumber: 137,
                                    columnNumber: 15
                                }, this))
                        }, void 0, false, {
                            fileName: "[project]/src/app/(main)/auction/page.tsx",
                            lineNumber: 135,
                            columnNumber: 11
                        }, this)
                    }, void 0, false, {
                        fileName: "[project]/src/app/(main)/auction/page.tsx",
                        lineNumber: 134,
                        columnNumber: 9
                    }, this),
                    /*#__PURE__*/ (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$compiled$2f$react$2f$jsx$2d$dev$2d$runtime$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["jsxDEV"])("button", {
                        onClick: handleSpin,
                        disabled: isSpinning || userPluses < 1,
                        className: "w-full h-14 flex items-center justify-center bg-gradient-to-r from-purple-600 to-blue-500 text-white text-lg font-bold rounded-xl  transition-all shadow-[0_4px_0_0_rgba(91,33,182,0.6)]  active:translate-y-1 active:shadow-[0_1px_0_0_rgba(91,33,182,0.6)] disabled:opacity-50 disabled:cursor-not-allowed",
                        children: isSpinning ? 'Крутится...' : 'Крутить'
                    }, void 0, false, {
                        fileName: "[project]/src/app/(main)/auction/page.tsx",
                        lineNumber: 147,
                        columnNumber: 9
                    }, this),
                    /*#__PURE__*/ (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$compiled$2f$react$2f$jsx$2d$dev$2d$runtime$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["jsxDEV"])("div", {
                        className: "text-sm text-purple-600 font-medium mt-2",
                        children: "Крутить стоит 1 плюс"
                    }, void 0, false, {
                        fileName: "[project]/src/app/(main)/auction/page.tsx",
                        lineNumber: 159,
                        columnNumber: 9
                    }, this)
                ]
            }, void 0, true, {
                fileName: "[project]/src/app/(main)/auction/page.tsx",
                lineNumber: 133,
                columnNumber: 7
            }, this),
            /*#__PURE__*/ (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$compiled$2f$react$2f$jsx$2d$dev$2d$runtime$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["jsxDEV"])("div", {
                className: "w-full bg-white rounded-xl shadow-sm p-5",
                children: [
                    /*#__PURE__*/ (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$compiled$2f$react$2f$jsx$2d$dev$2d$runtime$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["jsxDEV"])("h2", {
                        className: "text-lg font-bold mb-4",
                        children: "Премиум товар"
                    }, void 0, false, {
                        fileName: "[project]/src/app/(main)/auction/page.tsx",
                        lineNumber: 166,
                        columnNumber: 9
                    }, this),
                    /*#__PURE__*/ (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$compiled$2f$react$2f$jsx$2d$dev$2d$runtime$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["jsxDEV"])("div", {
                        className: "flex justify-between items-center py-3",
                        children: [
                            /*#__PURE__*/ (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$compiled$2f$react$2f$jsx$2d$dev$2d$runtime$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["jsxDEV"])("div", {
                                className: "text-left",
                                children: [
                                    /*#__PURE__*/ (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$compiled$2f$react$2f$jsx$2d$dev$2d$runtime$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["jsxDEV"])("div", {
                                        className: "font-medium",
                                        children: "Созвон с кумиром"
                                    }, void 0, false, {
                                        fileName: "[project]/src/app/(main)/auction/page.tsx",
                                        lineNumber: 170,
                                        columnNumber: 13
                                    }, this),
                                    /*#__PURE__*/ (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$compiled$2f$react$2f$jsx$2d$dev$2d$runtime$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["jsxDEV"])("div", {
                                        className: "text-sm text-gray-500",
                                        children: "30 минут личного общения"
                                    }, void 0, false, {
                                        fileName: "[project]/src/app/(main)/auction/page.tsx",
                                        lineNumber: 171,
                                        columnNumber: 13
                                    }, this)
                                ]
                            }, void 0, true, {
                                fileName: "[project]/src/app/(main)/auction/page.tsx",
                                lineNumber: 169,
                                columnNumber: 11
                            }, this),
                            /*#__PURE__*/ (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$compiled$2f$react$2f$jsx$2d$dev$2d$runtime$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["jsxDEV"])("div", {
                                className: "flex items-center",
                                children: [
                                    /*#__PURE__*/ (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$compiled$2f$react$2f$jsx$2d$dev$2d$runtime$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["jsxDEV"])("span", {
                                        className: "text-purple-600 font-bold mr-3",
                                        children: "10,000 плюсов"
                                    }, void 0, false, {
                                        fileName: "[project]/src/app/(main)/auction/page.tsx",
                                        lineNumber: 174,
                                        columnNumber: 13
                                    }, this),
                                    /*#__PURE__*/ (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$compiled$2f$react$2f$jsx$2d$dev$2d$runtime$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["jsxDEV"])("button", {
                                        disabled: userPluses < 10000,
                                        className: "bg-purple-600 text-white text-sm font-semibold px-3 py-1 rounded-lg disabled:opacity-50 disabled:cursor-not-allowed",
                                        children: "Купить"
                                    }, void 0, false, {
                                        fileName: "[project]/src/app/(main)/auction/page.tsx",
                                        lineNumber: 175,
                                        columnNumber: 13
                                    }, this)
                                ]
                            }, void 0, true, {
                                fileName: "[project]/src/app/(main)/auction/page.tsx",
                                lineNumber: 173,
                                columnNumber: 11
                            }, this)
                        ]
                    }, void 0, true, {
                        fileName: "[project]/src/app/(main)/auction/page.tsx",
                        lineNumber: 168,
                        columnNumber: 9
                    }, this)
                ]
            }, void 0, true, {
                fileName: "[project]/src/app/(main)/auction/page.tsx",
                lineNumber: 165,
                columnNumber: 7
            }, this)
        ]
    }, void 0, true, {
        fileName: "[project]/src/app/(main)/auction/page.tsx",
        lineNumber: 113,
        columnNumber: 5
    }, this);
}
_s(ShopPage, "Zd8vnLRRtMLvom6lb+nnWOLlqzE=");
_c = ShopPage;
var _c;
__turbopack_context__.k.register(_c, "ShopPage");
if (typeof globalThis.$RefreshHelpers$ === 'object' && globalThis.$RefreshHelpers !== null) {
    __turbopack_context__.k.registerExports(module, globalThis.$RefreshHelpers$);
}
}}),
}]);

//# sourceMappingURL=src_app_%28main%29_auction_page_tsx_e6ec22a1._.js.map