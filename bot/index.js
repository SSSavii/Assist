...
bot.onText(/\/admin$/, async (msg) => {
  ...
  try {
    ...
    // Формируем сообщение БЕЗ Markdown (plain text)
...
🎰 Участники розыгрышей (этот месяц):
...
bot.onText(/\/participants (\d+)/, async (msg, match) => {
  ...
  try {
    ...
    for (let i = 0; i < participants.length; i++) {
      ...
      const line = `${i + 1}. ${name} (${username})\n   В этом месяце: ${p.current_month_referrals} | Всего подписались: ${p.referral_count_subscribed || 0}\n\n`;
