...
export async function POST(req: NextRequest) {
    try {
        const { initData, awards } = await req.json();
        ...
        if (typeof awards !== 'string') {
            return NextResponse.json({ error: 'Awards must be a string' }, { status: 400 });
        }
        ...
        const stmt = db.prepare('UPDATE users SET awards = ? WHERE tg_id = ?');
        const result = stmt.run(awards, userData.id);
        ...
        console.error('API /api/profile/update-awards Error:', error);
